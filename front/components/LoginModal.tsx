...
import { LogIn, UserPlus, Loader2, ShieldCheck, Sparkles, ArrowRight, X } from 'lucide-react';
      ...
      <div className="relative w-full max-w-[400px] animate-in zoom-in-95 duration-300">
        <div className="ipad-glass rounded-md p-8 shadow-2xl shadow-indigo-500/10 border border-white/60 relative overflow-hidden">
          ...
          <button
            onClick={handleClose}
            disabled={loading}
            className="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-600 hover:bg-white/50 rounded-md transition-colors disabled:opacity-50"
            aria-label="关闭"
          >
            <X size={20} />
          </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-slate-900 hover:bg-indigo-600 text-white py-3.5 rounded-md font-black text-xs uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-2 group shadow-xl shadow-slate-300/50 hover:shadow-indigo-500/30 active:scale-95 disabled:opacity-50"
            >
              {loading ? (
          ...
          <div className="mt-6 pt-5 border-t border-slate-200/50 text-center">
            <button
              ...
              disabled={loading}
              className="text-slate-500 hover:text-indigo-600 text-[10px] font-black uppercase tracking-widest transition-colors flex items-center justify-center gap-2 mx-auto group disabled:opacity-50"
            >
