...
const CLASSIFICATION_RULES: ClassificationRule[] = [
  {
    ...
    keywords: [
      ...
      // 新增关键词
      '星光大赏', '国剧盛典', '演唱会', 'MV', 'mv', '音乐', '歌曲', '专辑', '单曲',
      '声生不息', '跨年', '红毯', '盛典', '颁奖', '颁奖礼', '颁奖典礼',
      '剧宣', '剧版', '热映', '上映', '首映', '点映', '路演',
      '超话', '工作室', '微博视频', '直播', '直播', '舞台', '表演',
      '杨紫', '檀健次', '白鹿', '丞磊', '易烊千玺', '贺峻霖', '肖战', '彭昱畅',
      '时代少年团', 'TF家族', 'EXO', 'RIIZE', 'R1SE', '陈楚生', '龚俊', '刘宇宁',
      '得闲谨制', '狙击蝴蝶', '长安二十四计', '莫离', '猎罪图鉴', '震耳欲聋',
      '主持人', '主持', '首秀', '压轴', '红毯', '盛典', '大赏'
    ],
    weight: 1.0,
  ...
  },
  {
    ...
    keywords: [
      ...
      '创业', '融资', 'IPO', '上市', '市值', '估值',
      // 新增关键词
      '高盛', '全球股市', '展望', '牛市', '行情', '5000点', '4500', '股市展望'
    ],
    weight: 1.0
  },
  {
    ...
    keywords: [
      ...
      // 新增关键词
      '小洛熙', '尸检', '手术', '离世', '女婴', '问责',
      '爱泼斯坦', '司法部', '文件', '公开', '涂黑', '克林顿',
      '法工委', '治安管理处罚法', '吸毒', '记录封存', '未成年人',
      '巴以冲突', '叙利亚', '戈兰高地', '恐怖组织', '以色列',
      '乌克兰', '俄罗斯', '普京', '泽连斯基', '选举', '克里米亚',
      '特朗普', '战舰', '总统', '对华政策', '中美关系',
      '信息茧房', '数据', '调查', '回应', '说明'
    ],
    weight: 1.0
...
function classifyContent(
  ...
  // 降低阈值，从0.5降到0.3，让更多内容能被正确分类
  // 同时，如果标题中包含平台标识（如"B站视频"、"微博视频"），即使得分较低也尝试分类
  const hasPlatformIndicator = title.toLowerCase().includes('b站') || 
                                title.toLowerCase().includes('微博') ||
                                title.toLowerCase().includes('抖音') ||
                                title.toLowerCase().includes('小红书');
  
  if (maxScore < 0.3 && !hasPlatformIndicator) {
    return 'other';
  }
  
  // 如果得分相同，优先选择更具体的分类（排除other）
  if (maxScore > 0) {
    ...
    if (tiedCategories.length > 0) {
      // 如果有多个相同得分的分类，选择第一个（通常是更早定义的，更通用的）
